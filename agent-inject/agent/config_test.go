...
func TestNewConfig(t *testing.T) {
	annotations := map[string]string{
		...
		"openbao.org/agent-inject-secret-different-path":                "different-path",
		...
		"openbao.org/agent-inject-secret-with-file-template":                  "with-file-template",
		fmt.Sprintf("%s-%s", AnnotationAgentInjectTemplateFile, "with-file-template"): "/etc/file-template",

		"openbao.org/agent-inject-template-just-template": "just-template1",
		"openbao.org/secret-volume-path-just-template":    "/custom/path",
		"openbao.org/agent-inject-command-just-template":  "/tmp/smth.sh",
		"openbao.org/agent-inject-file-just-template":     ".env",
		"openbao.org/agent-inject-perms-just-template":    "0600",

		"openbao.org/agent-inject-template-file-just-template-file": "just-template-file",

		"openbao.org/agent-inject-command-bar": "pkill -HUP app",

		"openbao.org/agent-inject-secret-baz":        "db/creds/baz",
		"openbao.org/agent-inject-template-baz":      `[[ with secret "db/creds/baz" ]][[ range $k, $v := .Data ]][[ $k ]]: [[ $v ]]\n[[ end ]][[ end ]]`,
		"openbao.org/agent-template-left-delim-baz":  "[[",
		"openbao.org/agent-template-right-delim-baz": "]]",
...
func TestFilePathAndName(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			map[string]string{
				"openbao.org/agent-inject-secret-foo": "db/creds/foo",
				"openbao.org/agent-inject-file-foo":   "nested/foofile",
			},
		...
		},
		{
			...
			map[string]string{
				"openbao.org/agent-inject-secret-foo": "db/creds/foo",
				"openbao.org/agent-inject-file-foo":   "/special/volume/foofile",
			},
		...
		},
		{
			...
			map[string]string{
				"openbao.org/agent-inject-secret-foo": "db/creds/foo",
				"openbao.org/agent-inject-file-foo":   "foofile_name_is_very_very_very_long",
				"openbao.org/secret-volume-path":      "/new/mount/path",
			},
		...
		},
		{
			...
			map[string]string{
				"openbao.org/agent-inject-secret-foo": "db/creds/foo",
				"openbao.org/agent-inject-file-foo":   "/special/foofile",
				"openbao.org/secret-volume-path":      "/new/mount/path",
			},
		...
		},
		{
			...
			map[string]string{
				"openbao.org/agent-inject-secret-foo": "db/creds/foo",
				"openbao.org/agent-inject-file-foo":   "nested/foofile",
				"openbao.org/secret-volume-path-foo":  "/new/mount/path",
			},
...
func TestFilePermission(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			map[string]string{
				"openbao.org/agent-inject-secret-foo": "db/creds/foo",
				"openbao.org/agent-inject-perms-foo":  "0600",
			},
			"0600",
		},
		{
...
func TestErrMissingKey(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			map[string]string{
				"openbao.org/agent-inject-secret-foo":  "db/creds/foo",
				"openbao.org/error-on-missing-key-foo": "true",
			},
			true,
		},
		{
		...
		},
		{
			...
			map[string]string{
				"openbao.org/agent-inject-secret-foo":  "db/creds/foo",
				"openbao.org/error-on-missing-key-foo": "false",
			},
			false,
		},
	}
...
func TestConfigTelemetry(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			map[string]string{
				"openbao.org/agent-telemetry-usage_gauge_period":                     "10m",
				"openbao.org/agent-telemetry-maximum_gauge_cardinality":              "500",
				"openbao.org/agent-telemetry-disable_hostname":                       "false",
				"openbao.org/agent-telemetry-enable_hostname_label":                  "false",
				"openbao.org/agent-telemetry-lease_metrics_epsilon":                  "1h",
				"openbao.org/agent-telemetry-num_lease_metrics_buckets":              "168",
				"openbao.org/agent-telemetry-add_lease_metrics_namespace_labels":     "false",
				"openbao.org/agent-telemetry-filter_default":                         "true",
				"openbao.org/agent-telemetry-statsite_address":                       "https://foo.com",
				"openbao.org/agent-telemetry-statsd_address":                         "https://foo.com",
				"openbao.org/agent-telemetry-circonus_api_token":                     "foo",
				"openbao.org/agent-telemetry-circonus_api_app":                       "nomad",
				"openbao.org/agent-telemetry-circonus_api_url":                       "https://api.circonus.com/v2",
				"openbao.org/agent-telemetry-circonus_submission_interval":           "10s",
				"openbao.org/agent-telemetry-circonus_submission_url":                "https://api.circonus.com/v2",
				"openbao.org/agent-telemetry-circonus_check_id":                      "foo",
				"openbao.org/agent-telemetry-circonus_check_force_metric_activation": "false",
				"openbao.org/agent-telemetry-circonus_check_instance_id":             "foo:bar",
				"openbao.org/agent-telemetry-circonus_check_search_tag":              "foo:bar",
				"openbao.org/agent-telemetry-circonus_check_display_name":            "foo",
				"openbao.org/agent-telemetry-circonus_check_tags":                    "foo,bar",
				"openbao.org/agent-telemetry-circonus_broker_id":                     "foo",
				"openbao.org/agent-telemetry-circonus_broker_select_tag":             "foo:bar",
				"openbao.org/agent-telemetry-dogstatsd_addr":                         "https://foo.com",
				"openbao.org/agent-telemetry-dogstatsd_tags":                         `["foo:bar", "foo:baz"]`,
				"openbao.org/agent-telemetry-prometheus_retention_time":              "24h",
				"openbao.org/agent-telemetry-stackdriver_project_id":                 "foo",
				"openbao.org/agent-telemetry-stackdriver_location":                   "useast-1",
				"openbao.org/agent-telemetry-stackdriver_namespace":                  "foo",
				"openbao.org/agent-telemetry-stackdriver_debug_logs":                 "false",
